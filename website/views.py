...
def home(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            login(request ,user)
            messages.success(request,"you have been logd in!")
            return redirect('home')
        else:
            messages.success(request,"There was an error Loggign in, Please try again.")
            return redirect('home')
    else:
...
def update_record(request, pk):
     if request.user.is_authenticated:
          ...
          form = AddRecordForm(request.POST or None, instance=customer_detail)
          if request.method == 'POST':
               if form.is_valid():
                    form.save()
                    messages.success(request,"Record has been Updated...")
                    return redirect('home')
          return render(request, 'update_record.html', {'form':form,'id':customer_detail.id})
     else:
          messages.error(request,"you must be logged In..")
          return redirect("home")